...
class CustomHopper(MujocoEnv, utils.EzPickle):
    def __init__(self, param, domain=None, distribution="uniform"):
    ...
    def sample_parameters(self):
        """Sample link masses from a specified distribution"""
        if self.distribution == "uniform":
            ...
            std = self.original_masses * 0.2  # 20% of original as std dev
            sampled = self.np_random.normal(loc=self.original_masses, scale=std)
            return np.clip(sampled, self.original_masses * 0.5, self.original_masses * 1.5)

        elif self.distribution == "lognormal":
            sigma = 0.3  # scale of lognormal
            log_original = np.log(self.original_masses)
            sampled = self.np_random.lognormal(mean=log_original, sigma=sigma)
            return np.clip(sampled, self.original_masses * 0.5, self.original_masses * 1.5)
